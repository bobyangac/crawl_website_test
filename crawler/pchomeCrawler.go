...
func (q *PChomeQuery) FindMaxPage(ctx context.Context, totalWebProduct int) (int, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "cannot generate request")
	}
	...
	if err != nil {
		return 0, errors.Wrapf(err, "failed to get response from %s", url)
	}
...
func (q *PChomeQuery) Crawl(ctx context.Context, page int, newProducts chan *Product, wgJob *sync.WaitGroup) {
	qSrc := q.GetQuerySrc()
