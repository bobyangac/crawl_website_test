...
type Job struct {
	...
	newProducts chan *crawler.Product
}
...
func Queue(ctx context.Context, keyWord string, pProduct chan pb.UserResponse, workerConfig WorkerConfig, jobsChan map[crawler.Web]chan *Job) {
	...
	newProducts := make(chan *crawler.Product, workerConfig.MaxProduct)
...
func send(ctx context.Context, wc crawler.Crawler, wgJob *sync.WaitGroup, newProducts chan *crawler.Product, jobChan chan *Job, workerConfig WorkerConfig) error {
